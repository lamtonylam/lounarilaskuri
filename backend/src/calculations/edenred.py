...
def calculate_optimal_payment(total_price):
    ...
    max_possible_uses = min(
        maximum_card_payment_times_per_day,
        int(total_price / minimum_lunch_card_payment),
    )
    ...
    for num_uses in range(1, max_possible_uses + 1):
        ...
        if max_card_total == total_price:
            ...
            if (
                avg_payment >= minimum_lunch_card_payment
                and avg_payment <= maximum_lunch_card_payment
            ):
                ...
                for i in range(num_uses):
                    if i == num_uses - 1:  # Last payment
                        ...
                        payment = min(
                            maximum_lunch_card_payment, remaining / (num_uses - i)
                        )
                        payment = max(minimum_lunch_card_payment, payment)

                    if (
                        payment >= minimum_lunch_card_payment
                        and payment <= maximum_lunch_card_payment
            ...
            if (
                ...
                for i in range(num_uses):
                    if i == num_uses - 1:
                        ...
                        payment = min(
                            maximum_lunch_card_payment, remaining_card / (num_uses - i)
                        )
                        payment = max(minimum_lunch_card_payment, payment)
    ...
    return {
        "card_payments": [
            round(p, 2) for p in different_cent_payments(best_card_payments)
        ],
...
def different_cent_payments(card_payments):
    if not card_payments:
        return []
    result = [round(card_payments[0], 2)]
    for i in range(1, len(card_payments)):
        next_payment = result[i - 1] - 0.01
        # Ensure we don't go below zero or original payment
        next_payment = max(0, min(card_payments[i], next_payment))
        result.append(round(next_payment, 2))
    return result
